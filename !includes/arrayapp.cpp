...
//DEFINITIONS
void array_fill(ITEM_TYPE a[], int low, int high, int total_num) {
    /*
    Purpose: fill up an empty array.
    Pre-condition: valid argument values.
    Post-condition: a[] has been given values.
    Dependency: GetNext()
    */

    //process...
    for (int i = 0; i < total_num; i++) {
        a[i] = GetNext(low, high);
    };
}
...
string error_description(ERRORS code)
{
   ...
   */
    ...
    case START_OF_LIST:
        ...
        break;
	case BAD_POS:
		description = "BAD CURSOR POSITION";
		break;
    case NO_ERROR:
    default:
        break;
...
void display_array_nav(ITEM_TYPE a[], int num_used, int cursor, ERRORS& error)
{
   ...
   * Note 11/2/17: changed setw from 2 to 3; commented out opening/closing brace
...
ERRORS sort(ITEM_TYPE a[], int num_used, bool descending) {
    /* Purpose: sort an array either ascending
          * or ascending.
          * Pre-condition: valid argument values.
          * Post-condition: array has been sorted.
          * Note: this is the algorithm we worked on in class.
          * TESTED: OCT 30, 2017
          */
    ...
    //process...
    if (num_used < 1) return EMPTY_ARRAY;
    for (int i = 1; i < num_used; i++) {
        //1.reset temp in each iteration
        temp = a[i];
        //2. inner loop
        for (int j = (i - 1); j >= 0; j--) {
            if ((!descending && a[j] > temp) || (descending && a[j] < temp))
            {
                swap(a[j], a[j + 1]);
            };
        };
